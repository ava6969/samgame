...
def daily_backtest(account_balances, daily_returns, metrics):
    ...
    d_sortino_ratio = qs.stats.sortino()
    d_calmar_ratio = qs.stats.calmar()
    d_recovery_factor = qs.stats.recovery_factor()
    d_risk_return_ratio = qs.stats.risk_return_ratio()
    d_max_drawdown = qs.stats.max_drawdown()
    d_drawdown_details = qs.stats.drawdown_details()
    d_alpha, d_beta = qs.stats.greeks(daily_returns, SPY / BTC? / cryptoETF?, periods = 7)
...
def monthly_backtest(account_balances, monthly_returns, metrics):
    ...
    m_sortino_ratio = qs.stats.sortino()
    m_calmar_ratio = qs.stats.calmar()
    m_recovery_factor = qs.stats.recovery_factor()
    m_risk_return_ratio = qs.stats.risk_return_ratio()
    m_max_drawdown = qs.stats.max_drawdown()
    m_drawdown_details = qs.stats.drawdown_details()
...
def annual_backtest(account_balances, annual_returns, metrics):
    ...
    a_sortino_ratio = qs.stats.sortino()
    a_calmar_ratio = qs.stats.calmar()
    a_recovery_factor = qs.stats.recovery_factor()
    a_risk_return_ratio = qs.stats.risk_return_ratio()
    a_max_drawdown = qs.stats.max_drawdown()
    a_drawdown_details = qs.stats.drawdown_details()
